...
public class CraterDensityFunction extends BlendedNoise {
    ...
    public static final KeyDispatchDataCodec<CraterDensityFunction> CODEC = KeyDispatchDataCodec.of(Codec
            .doubleRange(Double.MIN_VALUE, 1)
            .fieldOf("scale")
            .xmap(CraterDensityFunction::new, function -> function.scale)

    );

    public CraterDensityFunction(double scale) {
        this(new XoroshiroRandomSource(0L), scale);
    }
    public CraterDensityFunction(RandomSource randomSource, double scale) {
    ...
    public BlendedNoise withNewRandom(RandomSource random) {
        return new CraterDensityFunction(random, scale);
    }
    ...
    @Override
    public KeyDispatchDataCodec<? extends CraterDensityFunction> codec() {
        return CODEC;
    }
}